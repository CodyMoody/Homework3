...
// function for x in the Runge-Kutta Solver  (Not Necessary but is more intuitive)
__device__
float fx(float y) {
  return y;
}

// Runge-Kutta solver (Revision)
__device__
float2 rungeKutta(float x, float y, float dt, 
		  float tFinal, float param, float sys) {

  float k1x = 0.f, k2x = 0.f, k3x = 0.f, k4x = 0.f;
  float k1y = 0.f, k2y = 0.f, k3y = 0.f, k4y = 0.f;
  float dx = 0.f, dy = 0.f;
  ...
  for (float t = 0; t < tFinal; t += dt) {
    ...
    k1y = f(x, y, param, sys);
    k2x = fx(y + k1y*dt/2); //
    k2y = f(x + k1x*dt/2, y + k1y*dt/2, param, sys);
    k3x = fx(y + k2y*dt/2); //
    k3y = f(x + k2x*dt/2, y + k2y*dt/2, param, sys);
    k4x = fx(y + k3y*dt); //
    k4y = f(x + k3x*dt, y + k3y*dt, param, sys);

    dx = (k1x/6 + k2x/3 + k3x/3 + k4x/6)*dt;
    dy = (k1y/6 + k2y/3 + k3y/3 + k4y/6)*dt;
...
void stabImageKernel(uchar4 *d_out, int w, int h, float p, int s) {
  ...
  const float2 pos = rungeKutta(x0, y0, TIME_STEP, FINAL_TIME, p, s);
  const float dist_f = sqrt(pos.x*pos.x + pos.y*pos.y);
  // Report Dist_f
  if (x0 == scale((float)w*55/100,w) && y0 == scale((float)h*55/100,h))
  {
    printf("x0: %f, y0: %f, dist_f: %f\n", x0, y0, dist_f);
  }
...
void kernelLauncher(uchar4 *d_out, int w, int h, float p, int s) {
  ...
  printf("Kernel Compute Time in ms: %f\n", kernelTimeInMs);
}
